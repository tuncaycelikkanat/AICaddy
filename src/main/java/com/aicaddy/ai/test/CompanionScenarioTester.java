...
public class CompanionScenarioTester {
		...
		if (apiKey != null && !apiKey.isEmpty()) {
			try {
				...
				if (response.statusCode() == 200) {
					...
					if (com.aicaddy.ai.provider.GroqAiProvider.containsForeignOrHallucinatedWords(durumAnalizi)) {
						durumAnalizi = getScenarioAnalysis(sc.id());
					}
					if (com.aicaddy.ai.provider.GroqAiProvider.containsForeignOrHallucinatedWords(icDusunce)) {
						icDusunce = getScenarioThought(sc.id());
					}
					if (com.aicaddy.ai.provider.GroqAiProvider.containsForeignOrHallucinatedWords(finalReplik)) {
						finalReplik = getSimulatedReplik(sc);
					}

					// Apply strict Turkish, dictionary & category opening phrase rotation (Priority #1, #2, #4)
					durumAnalizi = com.aicaddy.ai.provider.GroqAiProvider.sanitizeTurkishText(durumAnalizi, sc.moodLabel(), sc.id());
					icDusunce = com.aicaddy.ai.provider.GroqAiProvider.sanitizeTurkishText(icDusunce, sc.moodLabel(), sc.id());
					finalReplik = com.aicaddy.ai.provider.GroqAiProvider.sanitizeTurkishText(finalReplik, sc.moodLabel(), sc.id());
	...
	public static List<Scenario> build50Scenarios() {
		...
		list.add(new Scenario(36, "FRUSTRATED", "FRUSTRATED", "Yoldaş uyarmasına rağmen oyuncu canı bitmek üzere olan elmas kazmayı tamir etmeden kırdı.", "Bıkmış ama sevecen"));
	...
	private static void generateMarkdownReport(List<TestResult> results, long totalDurationMs) {
		...
		for (TestResult r : results) {
			...
			md.append("- **💭 Yoldaş İç Düşünce**: *").append(r.icDusunce()).append("*\n");
			md.append("- **🗣️ AI Arkadaş Repliği**: > \"**").append(r.finalReplik()).append("**\"\n");
