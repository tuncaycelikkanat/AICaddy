...
public class GroqAiProvider implements AiProvider {
	...
	private synchronized String getApiKey() {
		...
		apiKey = ConfigManager.getGroqApiKey();
		return apiKey.isEmpty() ? null : apiKey;
	}

	// Allow runtime reload when config changes
	public synchronized void reloadApiKey() {
		apiKey = null;
	}

	private static final com.aicaddy.ai.resilience.CircuitBreaker GROQ_BREAKER =
			new com.aicaddy.ai.resilience.CircuitBreaker("GroqAPI", 3, 60_000L);

	@Override
	public CompletableFuture<String> generateResponseAsync(String systemPrompt, String userMessage) {
		return generateResponseAsync(null, systemPrompt, userMessage);
	}

	@Override
	public CompletableFuture<String> generateResponseAsync(ServerPlayer player, String systemPrompt, String userMessage) {
		...
		return HTTP_CLIENT.sendAsync(request, HttpResponse.BodyHandlers.ofLines())
				.thenApply(response -> {
					...
					return processSseStream(player, response.body());
				})
				.exceptionally(ex -> {
					GROQ_BREAKER.recordFailure();
	 ...
	 */
	private String processSseStream(ServerPlayer player, java.util.stream.Stream<String> lines) {
					...
					if (choices != null && !choices.isEmpty()) {
						...
						if (delta != null && delta.has("content")) {
							...
							while (spokenCount[0] < sentences.size()) {
								...
								// Use PlayerIsolatedTtsManager (per-player queue, no cross-player blocking)
								com.aicaddy.ai.tts.PlayerIsolatedTtsManager.speakSentenceAsync(player, sentenceToSpeak, isFirst[0]);
								spokenCount[0]++;
								isFirst[0] = false;
							}
						}
		...
		try {
			String rawJson = accumulatedJson.toString();
			com.aicaddy.ai.action.CompanionActionExecutor.queueActionsFromJson(rawJson);
			...
			if (parsed.has("ic_dusunce")) {
				ExampleMod.LOGGER.info("💭 [Yoldaş İç Düşünce]: {}", parsed.get("ic_dusunce").getAsString());
			}
		} catch (Exception ignored) {}
		...
		if (allSentences.isEmpty() && !fullReplik.isBlank()) {
			com.aicaddy.ai.tts.PlayerIsolatedTtsManager.speakSentenceAsync(null, fullReplik, isFirst[0]);
			...
			if (lastIdx >= 0) {
				...
				if (!remainder.isEmpty()) {
					com.aicaddy.ai.tts.PlayerIsolatedTtsManager.speakSentenceAsync(null, remainder, isFirst[0]);
				}
			}
		}
